...
# The below query might take over 30 seconds to complete and you might get an "Error Code: 2013. Lost connection to MySQL server during query" error
...
(rental_id, rental_last_update, customer_key, staff_key, film_key, store_key, rental_date_key, return_date_key, count_returns, count_rentals, rental_duration)
select
rental_id,
re.last_update,
customer_key,
...
film_key,
store_key,
dt.date_id AS rental_date_key,
rdt.date_id AS return_date_key,
sum(case when dt.date is not null then 1 else 0 end) as rentalcount,
sum(case when rdt.date is not null then 1 else 0 end) as returncount,
sum(film_rental_duration) as rental_duration
  from sakila.rental re
  join dim_customer cust on cust.customer_id = re.customer_id
  join dim_staff staff on re.staff_id = staff.staff_id
  join sakila.inventory inv on re.inventory_id = inv.inventory_id
  join dim_film film on inv.film_id = film.film_id
  join dim_store store on inv.store_id = store.store_id
  left join dim_date dt on DATE(dt.date) = DATE(re.rental_date)
  left join dim_date rdt on DATE(rdt.date) = DATE(re.return_date)
group by
	rental_id,
	re.last_update,
	customer_key,
